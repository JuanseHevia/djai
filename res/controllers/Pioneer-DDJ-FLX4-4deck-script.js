...
// Virtual deck routing for 4-deck toggle (physical left/right decks)
PioneerDDJFLX4.leftDeck = 1;
...
PioneerDDJFLX4.resolveDeckNumber = function(group) {
    if (group.indexOf("Channel1") !== -1) {
        return PioneerDDJFLX4.leftDeck;
    }
    if (group.indexOf("Channel2") !== -1) {
        return PioneerDDJFLX4.rightDeck;
    }
    return null;
};

PioneerDDJFLX4.resolveGroup = function(group) {
    const deck = PioneerDDJFLX4.resolveDeckNumber(group);
    if (!deck) {
        return group;
    }
    return group.replace(/Channel[12]/g, `Channel${deck}`);
};

PioneerDDJFLX4.setBeatFxChannel = function(side, enabled) {
    const deck = side === "left" ? PioneerDDJFLX4.leftDeck : PioneerDDJFLX4.rightDeck;
    engine.setValue("[EffectRack1_EffectUnit1]", `group_[Channel${deck}]_enable`, enabled);
};

PioneerDDJFLX4.applyBeatFxRouting = function() {
    PioneerDDJFLX4.setBeatFxChannel("left", PioneerDDJFLX4.beatFxChannelEnabled.left);
    PioneerDDJFLX4.setBeatFxChannel("right", PioneerDDJFLX4.beatFxChannelEnabled.right);
};

PioneerDDJFLX4.toggleDeckPair = function(side) {
    const isLeft = side === "left";
    const currentDeck = isLeft ? PioneerDDJFLX4.leftDeck : PioneerDDJFLX4.rightDeck;
    const nextDeck = isLeft ? (currentDeck === 1 ? 3 : 1) : (currentDeck === 2 ? 4 : 2);
    const prevDeck = currentDeck;

    if (isLeft) {
        PioneerDDJFLX4.leftDeck = nextDeck;
    } else {
        PioneerDDJFLX4.rightDeck = nextDeck;
    }
...
PioneerDDJFLX4.smartFxToggle = function(_channel, control, value) {
    ...
    if (control === 0x08) {
        PioneerDDJFLX4.toggleDeckPair("right");
        return;
    }
    if (control === 0x00) {
        PioneerDDJFLX4.toggleDeckPair("left");
        return;
    }
...
PioneerDDJFLX4.makeDeckControlHandler = function(controlName) {
    return function(_channel, _control, value, _status, group) {
        const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
        engine.setValue(resolvedGroup, controlName, value);
    };
};

PioneerDDJFLX4.handleHighResControl = function(controlName, value, control, group) {
    PioneerDDJFLX4.highResMSB[group] = PioneerDDJFLX4.highResMSB[group] || {};

    const isLsb = control >= 0x20;
    if (!isLsb) {
        PioneerDDJFLX4.highResMSB[group][controlName] = value;
        return;
    }

    const msb = PioneerDDJFLX4.highResMSB[group][controlName] || 0;
    const fullValue = (msb << 7) + value;
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);

    if (controlName === "pregain") {
        engine.setValue(
            resolvedGroup,
            controlName,
    ...
    engine.setValue(resolvedGroup, controlName, fullValue / 0x3FFF);
};

PioneerDDJFLX4.highResControlNames = [
    "pregain",
    "super1",
    "volume",
    "parameter1",
...
PioneerDDJFLX4.highResControlNames.forEach((name) => {
    PioneerDDJFLX4[name] = function(_channel, control, value, _status, group) {
        PioneerDDJFLX4.handleHighResControl(name, value, control, group);
    };
});

PioneerDDJFLX4.deckControlHandlers = {
    // trigger-only (press to fire, release ignored)
    LoadSelectedTrack: "trigger",
    start_play: "trigger",
    reloop_toggle: "trigger",
    "beatloop_0.25_toggle": "trigger",
    "beatloop_0.5_toggle": "trigger",
    "beatloop_1_toggle": "trigger",
    "beatloop_2_toggle": "trigger",
    "beatloop_4_toggle": "trigger",
    "beatloop_8_toggle": "trigger",
    "beatloop_16_toggle": "trigger",
    "beatloop_32_toggle": "trigger",
    "hotcue_1_clear": "trigger",
    "hotcue_2_clear": "trigger",
    "hotcue_3_clear": "trigger",
    "hotcue_4_clear": "trigger",
    "hotcue_5_clear": "trigger",
    "hotcue_6_clear": "trigger",
    "hotcue_7_clear": "trigger",
    "hotcue_8_clear": "trigger",

    // toggle (press toggles state)
    play: "toggle",
    pfl: "toggle",

    // momentary (press/release value)
    reverseroll: "momentary",
    cue_default: "momentary",
    loop_in: "momentary",
    loop_out: "momentary",
    reloop_andstop: "momentary",
    "hotcue_1_activate": "momentary",
    "hotcue_2_activate": "momentary",
    "hotcue_3_activate": "momentary",
    "hotcue_4_activate": "momentary",
    "hotcue_5_activate": "momentary",
    "hotcue_6_activate": "momentary",
    "hotcue_7_activate": "momentary",
    "hotcue_8_activate": "momentary",
};

Object.keys(PioneerDDJFLX4.deckControlHandlers).forEach((name) => {
    const mode = PioneerDDJFLX4.deckControlHandlers[name];
    PioneerDDJFLX4[name] = function(_channel, _control, value, _status, group) {
        const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
        if (mode === "toggle") {
            if (value) {
                script.toggleControl(resolvedGroup, name);
            }
            return;
        }
        if (mode === "trigger") {
            if (value) {
                engine.setValue(resolvedGroup, name, 1);
            }
            return;
        }
        engine.setValue(resolvedGroup, name, value);
    };
});
...
PioneerDDJFLX4.waveformZoom = function(midichan, control, value, status, group) {
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    if (value === 0x7f) {
        script.triggerControl(resolvedGroup, "waveform_zoom_up", 100);
    } else {
        script.triggerControl(resolvedGroup, "waveform_zoom_down", 100);
    }
};
...
PioneerDDJFLX4.beatFxChannel1 = function(_channel, control, value, _status, group) {
    ...
    PioneerDDJFLX4.beatFxChannelEnabled.left = enableChannel;
    PioneerDDJFLX4.applyBeatFxRouting();
};
...
PioneerDDJFLX4.beatFxChannel2 = function(_channel, control, value, _status, group) {
    ...
    if (value === 0x7f) { enableChannel = 1; }
    PioneerDDJFLX4.beatFxChannelEnabled.right = enableChannel;
    PioneerDDJFLX4.applyBeatFxRouting();
};
...
PioneerDDJFLX4.toggleLoopAdjustIn = function(channel, _control, value, _status, group) {
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    if (value === 0 || engine.getValue(resolvedGroup, "loop_enabled" === 0)) {
        return;
    }
...
PioneerDDJFLX4.cueLoopCallLeft = function(_channel, _control, value, _status, group) {
    if (value) {
        const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
        engine.setValue(resolvedGroup, "loop_scale", 0.5);
    }
};
...
PioneerDDJFLX4.cueLoopCallRight = function(_channel, _control, value, _status, group) {
    if (value) {
        const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
        engine.setValue(resolvedGroup, "loop_scale", 2.0);
    }
};
...
PioneerDDJFLX4.syncPressed = function(channel, control, value, status, group) {
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    if (engine.getValue(resolvedGroup, "sync_enabled") && value > 0) {
        engine.setValue(resolvedGroup, "sync_enabled", 0);
    } else {
        engine.setValue(resolvedGroup, "beatsync", value);
    }
};
...
PioneerDDJFLX4.syncLongPressed = function(channel, control, value, status, group) {
    if (value) {
        const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
        engine.setValue(resolvedGroup, "sync_enabled", 1);
    }
};
...
PioneerDDJFLX4.cycleTempoRange = function(_channel, _control, value, _status, group) {
    ...
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    const currRange = engine.getValue(resolvedGroup, "rateRange");
    let idx = 0;
    ...
    engine.setValue(resolvedGroup, "rateRange", this.tempoRanges[idx]);
};
...
PioneerDDJFLX4.jogTurn = function(channel, _control, value, _status, group) {
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    const deckNum = PioneerDDJFLX4.resolveDeckNumber(group) || channel + 1;
    // wheel center at 64; <64 rew >64 fwd
    let newVal = value - 64;

    // loop_in / out adjust
    const loopEnabled = engine.getValue(resolvedGroup, "loop_enabled");
    if (loopEnabled > 0) {
        if (PioneerDDJFLX4.loopAdjustIn[channel]) {
            newVal = newVal * PioneerDDJFLX4.loopAdjustMultiply + engine.getValue(resolvedGroup, "loop_start_position");
            engine.setValue(resolvedGroup, "loop_start_position", newVal);
            return;
        }
        if (PioneerDDJFLX4.loopAdjustOut[channel]) {
            newVal = newVal * PioneerDDJFLX4.loopAdjustMultiply + engine.getValue(resolvedGroup, "loop_end_position");
            engine.setValue(resolvedGroup, "loop_end_position", newVal);
            return;
        }
    }
    ...
    if (engine.isScratching(deckNum)) {
        ...
        engine.setValue(resolvedGroup, "jog", newVal * this.bendScale);
    }
};
...
PioneerDDJFLX4.jogSearch = function(_channel, _control, value, _status, group) {
    ...
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    engine.setValue(resolvedGroup, "jog", newVal);
};
...
PioneerDDJFLX4.beatjumpPadPressed = function(_channel, control, value, _status, group) {
    ...
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    engine.setValue(resolvedGroup, "beatjump_size", Math.abs(PioneerDDJFLX4.beatjumpSizeForPad[control]));
    engine.setValue(resolvedGroup, "beatjump", PioneerDDJFLX4.beatjumpSizeForPad[control]);
};
...
PioneerDDJFLX4.increaseBeatjumpSizes = function(_channel, control, value, _status, group) {
    ...
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    engine.setValue(resolvedGroup, "beatjump_size", PioneerDDJFLX4.beatjumpSizeForPad[0x21]);
};
...
PioneerDDJFLX4.samplerPadPressed = function(_channel, _control, value, _status, group) {
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    if (engine.getValue(resolvedGroup, "track_loaded")) {
        engine.setValue(resolvedGroup, "cue_gotoandplay", value);
    } else {
        engine.setValue(resolvedGroup, "LoadSelectedTrack", value);
    }
};

PioneerDDJFLX4.samplerPadShiftPressed = function(_channel, _control, value, _status, group) {
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    if (engine.getValue(resolvedGroup, "play")) {
        engine.setValue(resolvedGroup, "cue_gotoandstop", value);
    } else if (engine.getValue(resolvedGroup, "track_loaded")) {
        engine.setValue(resolvedGroup, "eject", value);
    }
};
...
PioneerDDJFLX4.toggleQuantize = function(_channel, _control, value, _status, group) {
    if (value) {
        const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
        script.toggleControl(resolvedGroup, "quantize");
    }
};
...
PioneerDDJFLX4.quickJumpForward = function(_channel, _control, value, _status, group) {
    if (value) {
        const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
        engine.setValue(resolvedGroup, "beatjump", PioneerDDJFLX4.quickJumpSize);
    }
};
...
PioneerDDJFLX4.stemMutePadPressed = function(_channel, control, value, _status, group) {
    ...
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    const stemCount = Math.min(engine.getValue(resolvedGroup, "stem_count"), 4);

    if (control - PioneerDDJFLX4.stemMutePadsFirstControl + 1 > stemCount) {
        return;
    }

    const stemGroup = `[${resolvedGroup.substring(1, resolvedGroup.length-1)}_Stem${control - PioneerDDJFLX4.stemMutePadsFirstControl + 1}]`;
...
PioneerDDJFLX4.stemMutePadShiftPressed = function(_channel, control, value, _status, group) {
    ...
    for (let stemIdx=1; stemIdx<=stemCount; stemIdx++) {
        const stemGroup = `[${resolvedGroup.substring(1, resolvedGroup.length-1)}_Stem${stemIdx}]`;
...
PioneerDDJFLX4.stemFxPadPressed = function(_channel, control, value, _status, group) {
    ...
    const resolvedGroup = PioneerDDJFLX4.resolveGroup(group);
    const stemGroup = `[QuickEffectRack1_[${resolvedGroup.substring(1, resolvedGroup.length-1)}_Stem${control - PioneerDDJFLX4.stemFxPadsFirstControl + 1}]]`;
...
PioneerDDJFLX4.pitchPadPressed = function(_channel, control, value, _status, group) {
    ...
    engine.setValue(resolvedGroup, "pitch_adjust", pitch);
};
...
PioneerDDJFLX4.pitchPadShiftPressed = function(_channel, control, value, _status, group) {
    ...
    let currentPitch = engine.getValue(resolvedGroup, "pitch_adjust");
    ...
    engine.setValue(resolvedGroup, "pitch_adjust", currentPitch);
};
